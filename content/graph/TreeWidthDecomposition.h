...
treeDecomp tree_decomposition(int n, vector<pii> edges) {
  ...
  treeDecomp ret{false};
  ret.par.resize(n, -1), ret.bags.resize(n);
  queue<int> que; rep(i, 0, n) if (sz(adj[i]) <= 2) que.push(i);
  auto rem_edge = [&](int u, int v) {
    adj[u].erase(adj[u].find(v));
    adj[v].erase(adj[v].find(u)); };
  int piv = 0; vi ord(n, -1);
  vector<pii> pcand(n, pii(-1, -1));
  while (sz(que)) {
    int x = que.front(); que.pop();
    if (ord[x] != -1) continue;
    ret.bags[x].push_back(x); ord[x] = piv++;
    if (sz(adj[x]) == 1) {
      int y = *adj[x].begin(); rem_edge(x, y);
      ret.bags[x].push_back(y);
      if (sz(adj[y]) <= 2) que.push(y);
      pcand[x] = pii(y, y);
    } else if (sz(adj[x]) == 2) {
      int u = *adj[x].begin(), v = *adj[x].rbegin();
      rem_edge(x, u), rem_edge(x, v);
      adj[u].insert(v), adj[v].insert(u);
      ret.bags[x].push_back(u), ret.bags[x].push_back(v);
  ...
  rep(i, 0, n) {
    ...
    } else ret.par[i] = (ord[x] < ord[y]) ? x : y; }
  return ret; }